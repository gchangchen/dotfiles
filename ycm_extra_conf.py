  ...
  if language == 'cfamily':
    ...
    if extension in [ '.h', '.hxx', '.hpp', '.hh' ]:
      ...
      if os.path.exists( basename + '.c'):
        extension = '.c'
      elif os.path.exists( basename + '.m') or os.path.exists( basename + '.mm'):
        extension = '.m'
      else:
        extension = '.cc'

    if language_dict[ extension ]:
      flags.append( '-x' )
      flags.append( language_dict[ extension ] )

      if language_dict[ extension ] == 'c':
        flags.append( '-std=c99' )
        flags.append( '-isystem' )
        flags.append( '/usr/include')

      if language_dict[ extension ] == 'c++':
        flags.append( '-std=c++11' )
        flags.append( '-isystem' )
        flags.append( '/usr/include/c++/' + os.popen('gcc -dumpversion').read().rstrip('\n') )
